...
var searchLastCity = document.getElementById('saved-cities')

searchLastCity.addEventListener('click', getDataFromSearchPartTwo);
...
function renderLastCity() {
    ...
    if (lastCitySaved !== null) {
        searchLastCity.innerHTML = lastCitySaved;
    } else {
        return;
    }
}
...
function getDataFromSearchPartTwo () {
    var cityText = searchLastCity.textContent;  
    locationByName = 'http://api.openweathermap.org/geo/1.0/direct?q=' + cityText + '&limit=5&appid=' + APIkey ;
    function convertToLatAndLon() {
        fetch(locationByName)
        .then(function(response){
